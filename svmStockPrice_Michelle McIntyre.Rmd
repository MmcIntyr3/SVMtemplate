...
## Stock selected for this exercise:SBUX 
...
## get data:
names of variables in SBUX Data
...
```{r}
SBUXData = read.csv('SBUX stock pricetrim.csv')
attach(SBUXData)
# set up data frame
SBUXDataFrame = data.frame(open=price.open, high=price.high,
...
```{r}
x <- subset(SBUXDataFrame, select = -change)
y <- SBUXDataFrame$change
model1 <- svm(x, y) 
...
plot(cmdscale(dist(SBUXDataFrame[,-7])),
     col = as.integer(SBUXDataFrame[,7]),xlab="normalized X", ylab = "$ change",
...
XtrainSet = SBUXDataFrame[TrainSetIndex,-7]
YtrainSet = SBUXDataFrame$change[TrainSetIndex]
```
...
```{r}
XtestSet = SBUXDataFrame[(nTrain:nObs),-7]
YtestSet = SBUXDataFrame$change[nTrain:nObs]
```
...
I used Starbucks
  ...
  I used starbucks, because I love them and I'm there at least 2-3 times a week; therefore, I wanted to know how well the company is doing with the purchase of my $5 drink.
...
<!-- There are 5 days available for prediction; if the daily prediction was 'U' buy the stock, then cash out when/if price increases within 5 days. If the daily predictions is 'D', sell the shock short and cash out within 5 days.  If the prediction is E, no investment.  Include your ROI results !-->  
  ...
  Of course I'd put my money here, I am already, so why not make some money from the money I'm putting into the company. 